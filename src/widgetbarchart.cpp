 ...
 *
 * Set the style and the initialize the municipality and year selection widgets
 *
 * @param parent is the Display Window widget
 */
...
/**
 * @brief Construct a line chart widget of a given parent, active measure, low and high range
 *
 * Also create connections to allow users to modicy chart appearance dynamically
 *
 * @param parent chosen parent
 * @param meas  active measure
 * @param range low/high range of the data
 */
widgetBarChart::widgetBarChart(QWidget *parent, const Measure &meas, const std::pair<double, double> &range):QWidget(parent),ui(new Ui::widgetBarChart),_meas(new Measure(meas)), _currentTemp(new GraphTemplate())
{
    ui->setupUi(this);
    _range =range;
    this->setStyleSheet(
                ...
                "QPushButton#pushButtonLoadTemp:hover {"
                "background-color: rgb(130,130,180); }"

                "QPushButton#pushButtonLoadTemp {"
                "background-color: rgb(125,125,175);"
                "border-style: outset;"
                "border-width: 2px;"
                "border-color: rgb(50,50,75);"
                ...
                "QPushButton#pushButtonLoadTemp:hover:pressed {"
                "background-color: rgb(100,100,145);"
                "border-style:inset;}"
                ...
                "QPushButton#pushButtonSaveTemp {"
                "background-color: rgb(125,125,175);"
                "border-style: outset;"
                "border-width: 2px;"
                "border-color: rgb(50,50,75);"
                ...
                "QPushButton#pushButtonSaveTemp:hover:pressed {"
                "background-color: rgb(100,100,145);"
                "border-style:inset;}"

                "QPushButton#pushButtonSaveTemp:hover {"
                "background-color: rgb(130,130,180); }"
 ...
 *
 * Obtain the range from the measure and add the specific data from the years that are selected to a vector
 * Then draw all the points in a bar chart.
 *
...
/**
 * @brief Draw a bar chart using the selected years and Municipalities
 */
 ...
 * @brief Draw a bar chart using the selected years and Municipalities
 *
 * Draw using with default features such as labels and background color.
 */
...
/**
 * @brief Enables the user to edit the title.
...
/**
 * @brief Enables the user to edit the titles of the axis.
...
/**
 * @brief Enables the user to change the name of the item in the legend.
 * @param legend
 * @param item
 */
...
/**
 * @brief Keeps track of selection changes made by user
 *
 * This function allows the user to move the graph in two directions at once,
 * or to change two axies at once, or two legend items at once.
 */
...
/**
 * @brief Allow user to drag around plots in the chart.
 */
...
/**
 * @brief Keep track of position of mouse to display appropriate menu.
...
/**
 * @brief Set background of graph with an image selected by a user.
 */
...
/**
 * @brief Enable the user to select an image and set it as axis background of graph.
 */
...
/**
 * @brief Enable the user to change the color of the legend.
 */
...
/**
 * @brief Change the color of the selected graph or the background of the axis graph if it was seleced.
 */
...
/**
 * @brief Let the user change the color of the axis background.
 */
...
/**
 * @brief Let the user change the color of the whole plot
 */
...
/**
 * @brief Let the user change the location of the legend
 */
...
/**
 * @brief Draw a line on the graph representing the average for the _measure
 * @param activeMunis
 */
...
/**
 * @brief widgetBarChart::setImageBackground set graph bg image
 * @param path path to the image
 */
void widgetBarChart::setImageBackground(const QString &path)
{
...
/**
 * @brief widgetBarChart::setBGAxisImage set bg axis image
 * @param path path to the image
 */
void widgetBarChart::setBGAxisImage(const QString &path)
{
    QPixmap pixmap1(path);
    QPalette p = palette();
...
/**
 * @brief widgetBarChart::setBGColour set graph bg colour
 * @param col the colour
 */
void widgetBarChart::setBGColour(const QColor &col)
{
    ui->widgetBarChart_2->setBackground(QBrush(col));
    ui->widgetBarChart_2->replot();
}
/**
 * @brief widgetBarChart::setAxisBGColor set axis background colour
 * @param col the colour
 */
void widgetBarChart::setAxisBGColor(const QColor &col)
{
    ui->widgetBarChart_2->axisRect()->setBackground(QBrush(col));
    ui->widgetBarChart_2->replot();
}

/**
 * @brief widgetBarChart::setLegendColor set the legend colour
 * @param col the colour
 */
void widgetBarChart::setLegendColor(const QColor &col)
{
    ui->widgetBarChart_2->legend->setBrush(QBrush(col));
    ui->widgetBarChart_2->replot();
}
/**
 * @brief widgetBarChart::setLegendPos set the position of the legend
 * @param pos the position
 */
void widgetBarChart::setLegendPos(const int &pos)
{
    ui->widgetBarChart_2->axisRect()->insetLayout()->setInsetAlignment(0, (Qt::Alignment)pos);
    ui->widgetBarChart_2->replot();
}

/**
 * @brief Redraw the graph if a municipality is selected or deselected
 */
...
/**
 * @brief Draw a line representing the average for the measure.
 * @param checked
 */
...
/**
 * @brief Redraw the graph back to default features.
 */
...
/**
 * @author Jen Stewart
 *  @brief Save the Bar Chart as an image file
 *
 * Using the static QfileDialog box to allow the user to select a file,
 * write to that file in the format specified,
 * and save the directory they saved the file in to allow them to save many files in the same location.
 *
 */
...
/**
 * @brief widgetBarChart::on_pushButtonLoadTemp_clicked Load preferences
 */
void widgetBarChart::on_pushButtonLoadTemp_clicked()
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open File"),
                                                    "",
    ...
    _currentTemp->parseInPrefs(FilterData::loadTemplate(file));

    std::cout << fileName.toStdString() << std::endl;



    setAxisBGColor(_currentTemp->bgAxisColour());
    setBGColour(_currentTemp->bgColour());
    setImageBackground(_currentTemp->bgImage());
    setBGAxisImage(_currentTemp->bgAxisImg());
    setLegendColor(_currentTemp->bgLegendColour());
    setLegendPos(_currentTemp->legendPos());
}
/**
 * @brief widgetBarChart::on_pushButtonSaveTemp_clicked save
 */
void widgetBarChart::on_pushButtonSaveTemp_clicked()
{
    QString fileName = QFileDialog::getSaveFileName(this,"Save File", _dirname,"*.tpl");
    QFile file(fileName);
    if (!file.open(QIODevice::ReadWrite) ) return;

    FilterData::saveTemplate(_currentTemp->getStringPrefs(),file);
    file.close();
}
